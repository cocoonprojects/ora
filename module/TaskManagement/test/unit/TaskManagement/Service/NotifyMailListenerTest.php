...
class NotifyMailListenerTest extends \PHPUnit_Framework_TestCase {
	...
	/*public function testSendMail(){
		...
		$this->task->method('getSubject')->willReturn('TaskSubject_Test');
		$this->task->method('getOwner')->willReturn('60000000-0000-0000-0000-000000000000');
		
		//MockOwner
		$this->owner = $this->getMockBuilder(User::class)->getMock();
		$this->owner->method('getEmail')->willReturn('test_owner@mail.com');
		$this->owner->method('getFirstname')->willReturn('OwnerFirstname');
		
		//MockMember
		$this->member = $this->getMockBuilder(User::class)->getMock();
		$this->member->method('getFirstname')->willReturn('MemberFirstname');
		$this->member->method('getLastname')->willReturn('MemberLastname');

		$this->trigger = 'estimation';
		
		$this->assertTrue($this->notificationMailListener->sendMail($this->task, $this->member, $this->trigger));
	}
	
	public function testSendMailWithNullTask(){
		...
		$this->assertFalse($this->notificationMailListener->sendMail(null, $this->member, $trigger));
		
	}
	
	public function testSendMailWithNullMember(){
		//MockTask
		$this->task = $this->getMockBuilder(Task::class)->getMock();
		$this->task->method('getSubject')->willReturn('TaskSubject_Test');
		$this->task->method('getOwner')->willReturn('60000000-0000-0000-0000-000000000000');
		...
		$this->assertFalse($this->notificationMailListener->sendMail($this->task, null, $this->trigger));
	}
	
	public function testSendMailWithNullTrigger(){
		//MockTask
		$this->task = $this->getMockBuilder(Task::class)->getMock();
		$this->task->method('getSubject')->willReturn('TaskSubject_Test');
		$this->task->method('getOwner')->willReturn('60000000-0000-0000-0000-000000000000');
		...
		$this->assertFalse($this->notificationMailListener->sendMail($this->task, $this->member, null));
	}
	
	public function testSendMailWithWrongTrigger(){
		//MockTask
		$this->task = $this->getMockBuilder(Task::class)->getMock();
		$this->task->method('getSubject')->willReturn('TaskSubject_Test');
		$this->task->method('getOwner')->willReturn('60000000-0000-0000-0000-000000000000');
		...
		$this->assertFalse($this->notificationMailListener->sendMail($this->task, $this->member, $this->trigger));
	}
	
	/*public function testSendMail_(){
		$task = $this->getMockBuilder(Task::class)->getMock();
		
		$owner = User::create();
		$owner->setFirstname('ownerName');
		$owner->setEmail('ownerMail@mail.com');
		
		$member = User::create();
		$member->setFirstname('memberFistname');
		$member->setLastname('memberLastname');
		
		$task = $this->getMockClass(Task::class)->getMock();
		$task = Task::create(null, 'subjectTask', $owner);
