...
class TaskCommandsListener extends ReadModelProjector {
	...
	protected function onTaskCreated(StreamEvent $event) {
		...
		if ($type == WriteModelTask::class) {
			...
			$entity->setStatus ( $event->payload ()['status'] )->setCreatedAt ( $event->occurredOn () )->setCreatedBy ( $createdBy )->setMostRecentEditAt ( $event->occurredOn () )->setMostRecentEditBy ( $createdBy );
	...
	protected function onEstimationAdded(StreamEvent $event) {
		...
		$id = $event->metadata ()['aggregate_id'];
		$taskMember = $this->entityManager->find ( TaskMember::class, [ 
				'task' => $id,
				'user' => $user 
		] );
