	...
	onLoadPeopleCompleted: function(json)
	{
		...
		if ($(members).length == 0) {
			...
			$.each(members, function(key, member) {
				container.append('<li style="margin-bottom: 5px"><img src="' + member.picture + '" style="max-width: 60px; max-height: 60px;" class="img-circle"> <a href="user-profile/'+member.id+'" data-action="user-detail" data-user=' + member.id + '"><span class="firstname">' + member.firstname + '</span> <span class="lastname">' + member.lastname + '</span></a></li>')
			});
		}
	}
};
