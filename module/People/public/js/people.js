	...
	onLoadPeopleCompleted: function()
	{
		...
		if ($(members).length == 0) {
			...
			$.each(members, function(key, member) {
				container.append('<li><img src="' + member.picture + '" style="max-width: 60px; max-height: 60px;" class="img-circle"> <a href="#" data-url="people/users/' + member.id + '"><span class="firstname">' + member.firstname + '</span> <span class="lastname">' + member.lastname + '</span></a></li>')
			});
		}
	}
};
