	...
	public function attach(EventManagerInterface $events) {
		...
		$this->listeners [] = $events->getSharedManager ()->attach ( Application::class, TaskCreated::class, function (Event $event) {
			...
			if ($streamEvent->metadata ()['aggregate_type'] == KanbanizeTask::class) {
				...
				$entity->setTaskId ( $streamEvent->payload ()['taskid'] )->setSubject ( $streamEvent->payload ()['subject'] )->setColumnName ( $streamEvent->payload ()['columnname'] )->setStatus ( $streamEvent->payload ()['status'] )->setCreatedAt ( $streamEvent->occurredOn () )->setCreatedBy ( $createdBy )->setMostRecentEditAt ( $streamEvent->occurredOn () )->setMostRecentEditBy ( $createdBy );
				$this->entityManager->persist ( $entity );
				$this->entityManager->flush ( $entity );
			}
		}, 200 );
		
		$this->listeners [] = $events->getSharedManager ()->attach ( Application::class, TaskUpdated::class, function (Event $event) {
			$streamEvent = $event->getTarget ();
			if ($streamEvent->metadata ()['aggregate_type'] == KanbanizeTask::class) {
				$id = $streamEvent->metadata ()['aggregate_id'];
