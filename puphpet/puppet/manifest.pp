  ...
  if count($apache_vhosts) > 0 {
    each( $apache_vhosts ) |$key, $vhost| {
      ...
      create_resources(apache::vhost, { "${key}" => merge($vhost, {
          ...
          'ssl_cert'        => array_key_true($vhost, 'ssl_cert') ? { true => $vhost['ssl_cert'],      default => undef },
          'ssl_key'         => array_key_true($vhost, 'ssl_cert') ? { true => $vhost['ssl_key'],       default => undef },
          'ssl_chain'       => array_key_true($vhost, 'ssl_cert') ? { true => $vhost['ssl_chain'],     default => undef },
          'ssl_certs_dir'   => array_key_true($vhost, 'ssl_cert') ? { true => $vhost['ssl_certs_dir'], default => undef }
        })
      })
